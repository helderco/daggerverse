...
type Codegen = dagger.Codegen

// CodegenIntrospectOpts contains options for Codegen.Introspect
type CodegenIntrospectOpts = dagger.CodegenIntrospectOpts

// An enum type's value
type CodegenEnumValue = dagger.CodegenEnumValue

// Experimental alternative to the introspection query, using the TypeDef API
type CodegenExperimental = dagger.CodegenExperimental

// An object type's field
type CodegenField = dagger.CodegenField

// An argument or input object type's field
type CodegenInputValue = dagger.CodegenInputValue

type CodegenSchema = dagger.CodegenSchema

// A type in the API
type CodegenType = dagger.CodegenType

// Type information
//
// Can be the return type of a field, the type of a field argument,
// or the underlying type in a list or non-null type.
type CodegenTypeRef = dagger.CodegenTypeRef

type WithCodegenTypeRefFunc = dagger.WithCodegenTypeRefFunc
...
func (r *GoPythonCodegen) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	switch parentName {
	case "GoPythonCodegen":
		switch fnName {
		case "ContainerEcho":
			var parent GoPythonCodegen
			...
			return (*GoPythonCodegen).ContainerEcho(&parent, stringArg), nil
		case "GrepDir":
			var parent GoPythonCodegen
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*GoPythonCodegen).GrepDir(&parent, ctx, directoryArg, pattern)
		default:
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("GoPythonCodegen").
					WithFunction(
